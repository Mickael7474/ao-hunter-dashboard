...
| 2026-04-05 | app.py | Build Render crash "Exited with status 1" : doublon fonction `api_batch_generer` (2 routes `/api/batch/generer` et `/api/batch-generer` avec le meme nom Python). Renomme la 2e en `api_batch_generer_smart`. | Resolu |
| 2026-04-05 | export_docx_render.py | Headings ##/### apparaissent en texte brut dans les .docx : `startswith("# ")` ne matchait que les titres `#` (niveau 1). Corrige en `startswith("#")` pour matcher tous les niveaux. | Resolu |
