/**
 * AO Hunter - Auto-remplissage profil Almera sur les plateformes de depot
 *
 * Detecte la plateforme (PLACE, AWS, marches-securises, e-marchespublics)
 * et remplit automatiquement les champs de formulaire avec les infos Almera.
 *
 * JAMAIS de soumission automatique - remplissage uniquement.
 */
...
(function() {
    ...
    // --- Mapping champs par strategie ---
    // Chaque entree : { valeur, patterns_name, patterns_id, patterns_label, patterns_placeholder }
    ...
    function getLabelForField(field) {
        // Cherche le <label> associe par for= ou par parenthese
        if (field.id) {
    ...
    function setFieldValue(field, value) {
        ...
        if (currentVal.trim() !== '' && currentVal.trim() !== value.trim()) {
            // Ne pas ecraser une valeur differente deja presente - on remplit quand meme
            // car l'utilisateur veut auto-remplir
        }
        field.value = value;
        // Trigger events pour les frameworks JS
    ...
    function detectPlateforme() {
        const url = window.location.href.toLowerCase();
        if (url.includes('marches-publics.gouv.fr') || url.includes('place.gouv') || url.includes('maximilien.fr')) {
            return 'place';
        }
        if (url.includes('achatpublic.com') || url.includes('achats.defense.gouv') || url.includes('aws-defense')) {
            return 'aws';
        }
        if (url.includes('marches-securises.fr')) {
            return 'marches-securises';
        }
        if (url.includes('e-marchespublics.com') || url.includes('marchespublics.com')) {
            return 'e-marchespublics';
        }
        return 'inconnu';
    }
    ...
    function remplirFormulaire() {
        ...
        // Gerer les selects (pays notamment)
        ...
        return { filled, total, filledFields };
    }

    // --- Affichage compteur ---

    function showCounter(filled, total) {
        ...
        const plateforme = detectPlateforme();
        counter.innerHTML = '<strong>Almera AutoFill</strong><br>' +
            filled + ' champs remplis sur ' + total + ' detectes' +
            '<br><small>Plateforme : ' + plateforme + '</small>' +
            '<br><small style="opacity:0.7">Cliquez pour fermer</small>';

        // Auto-disparition apres 10s
        setTimeout(function() {
    ...
    function almeraAutofill() {
        const result = remplirFormulaire();
        showCounter(result.filled, result.total);
        return result;
    }

    // Exposer globalement
    window.almeraAutofill = almeraAutofill;
    window.ALMERA_PROFIL = PROFIL;
